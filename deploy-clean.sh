...
# Setup database (skip for now, we'll handle manually)
echo "⚠️  Database setup skipped - will be handled manually"
...
server {
    listen 80;
    server_name tlsoftware.io.vn www.tlsoftware.io.vn;
...
# Test API
if curl -s -I http://localhost:5001 2>/dev/null | grep "HTTP" > /dev/null; then
...
echo "   2. Setup database connection and run migrations"
echo "   3. Test login at: http://$DOMAIN/Auth/Login"  
